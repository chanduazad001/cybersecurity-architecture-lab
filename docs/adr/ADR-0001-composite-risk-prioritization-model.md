...
- **Quality of prioritization** — whether the model can combine severity, real-world exploitability, and Sreelaxmi's own asset/client context, or only reflects what one vendor's scanner happens to expose
...
- **Ability to weight client/BFSI-in-scope criticality** — a factor no scanner vendor can know without Sreelaxmi's own CMDB context
...
- **Cons:** ties prioritization logic directly to scanner choice — a future scanner change (or a second scan source, e.g. container/CSPM findings) forces the prioritization model to change too, violating the intended Physical-vs-Logical separation; the scoring methodology is typically a vendor black box, hard to explain confidently to auditors or clients when asked why a specific finding was prioritized as it was; cannot incorporate Sreelaxmi's own CMDB asset-criticality or BFSI-in-scope weighting, since the vendor has no visibility into Sreelaxmi's client contracts
...
- **Pros:** decouples prioritization logic from scanner choice entirely — the scanner is just one of several API inputs, so [ADR-0002](ADR-0002-scanner-selection-tenable-nessus.md) can change independently in the future; can directly encode the **Prioritized** attribute by weighting BFSI/client-facing asset criticality from the CMDB; scoring rationale is fully owned and documented by Sreelaxmi, so it is auditable and explainable on demand; extensible to new inputs later (e.g., KEV status, the PCI-DSS-as-a-service segmentation findings in [ADR-0004](ADR-0004-multi-tenant-data-isolation-model.md))
